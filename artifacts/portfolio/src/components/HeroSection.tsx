...
export const HeroSection = () => {
  ...
  useEffect(() => {
    ...
    const createStarField = () => {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        for (let j = 0; j < starCount; j++) {
          ...
          else if (colorChoice < 0.9) color.setHSL(0.66, 0.8, 0.65); // cyan/indigo
          else color.setHSL(0.55, 0.8, 0.6);
    ...
    const createNebula = () => {
      ...
      const material = new THREE.ShaderMaterial({
        uniforms: { time: { value: 0 }, color1: { value: new THREE.Color(0x0a192f) }, color2: { value: new THREE.Color(0x3b82f6) }, opacity: { value: 0.3 } },
        vertexShader: `varying vec2 vUv; varying float vElevation; uniform float time;
          void main() { vUv = uv; vec3 pos = position;
            float elevation = sin(pos.x * 0.01 + time) * cos(pos.y * 0.01 + time) * 20.0;
            pos.z += elevation; vElevation = elevation;
            gl_Position = projectionMatrix * modelViewMatrix * vec4(pos, 1.0); }`,
        fragmentShader: `uniform vec3 color1; uniform vec3 color2; uniform float opacity; uniform float time;
    ...
    const createMountains = () => {
      const { current: refs } = threeRefs;
      const layers = [
        { distance: -50, height: 60, color: 0x050814, opacity: 1 },
        { distance: -100, height: 80, color: 0x0a1024, opacity: 0.8 },
        { distance: -150, height: 100, color: 0x0f172a, opacity: 0.6 },
        { distance: -200, height: 120, color: 0x1e293b, opacity: 0.4 }
      ];
    ...
    const createAtmosphere = () => {
      ...
      const material = new THREE.ShaderMaterial({
        ...
        fragmentShader: `varying vec3 vNormal; varying vec3 vPosition; uniform float time;
          void main() {
            float intensity = pow(0.7 - dot(vNormal, vec3(0.0, 0.0, 1.0)), 2.0);
            vec3 atmosphere = vec3(0.1, 0.3, 0.8) * intensity;
  ...
  return (
    <div ref={containerRef} className="hero-container cosmos-style">
      ...
      <div ref={scrollProgressRef} className="scroll-progress" style={{ visibility: 'hidden' }}>
        ...
        <div className="section-counter font-mono">

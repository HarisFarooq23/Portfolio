...
export function TechCard({ label, abbr, skills, colors, className }: TechCardProps) {
  return (
    <div className={cn("group relative overflow-hidden border border-[rgba(201,176,140,0.15)] bg-[#0a0907] cursor-default transition-all duration-300 hover:border-[rgba(201,176,140,0.4)]", className)}>
      <PixelCanvas colors={colors} gap={4} speed={25} />
      <div className="relative z-10 p-8">
        <div className="text-5xl font-black tracking-tighter text-[rgba(201,176,140,0.15)] group-hover:text-[rgba(201,176,140,0.3)] transition-colors duration-300 mb-4 font-mono select-none">
          {abbr}
        </div>
        <div className="text-sm font-mono tracking-widest uppercase text-[#c9b08c] mb-4 opacity-70">{label}</div>
        <div className="flex flex-wrap gap-2">
          {skills.map(s => (
            <span key={s} className="px-2 py-1 text-xs font-mono border border-[rgba(201,176,140,0.2)] text-[rgba(237,224,204,0.7)] rounded-[2px]">
              {s}
            </span>
          ))}
        </div>
